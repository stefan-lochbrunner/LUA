...
local DST
local GMToffset
local dateFormat      -- hh:mm / hh:mm:ss / full date (returns conplete string) / debug (returns payload)
-- outputs
local time            -- in format specified by dateFormat
local hours
local minutes
local seconds
...
-- fetch time
local function update(GMToffset, dateFormat)
  ...
  conn:on("receive", function(conn, payload)
    ...
    if(dateFormat == 3) then      --return full date
...
function getTime()
  update()
  tmr.delay(1000)   --time to establish connection and receive answer
  return time
end
